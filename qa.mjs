...
test('JSONエクスポートはバージョンと全データを含む', () => {
  const exported=JSON.parse(api.exportedJson());equal(exported.schemaVersion,7);truthy(Array.isArray(exported.trainingThemes));truthy(Array.isArray(exported.rewardHistory));truthy(Array.isArray(exported.cashUsageHistory));truthy(exported.settings);
});
